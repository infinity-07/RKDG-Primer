...
\begin{itemize}
    ...
    \item HLL Flux
          ...
          \begin{equation}
              \mathbf{F}_{i+1 / 2}^{H l l}=\left\{\begin{array}{l}
...
\end{equation}
所以，相比于 WENO-JS, WENO-Z 会增加间断模板的权重。
\subsubsection{WENO-Z+}
\subsection{自由设置线性权}
\subsection{HWENO}
...
\subsection{稀疏波}
\subsection{接触间断}
总而言之：接触波是一种不连续的波，在其上压力和粒子速度都是恒定的，但密度不连续地跳跃，依赖于密度的变量，如比内能、温度、声速、熵等，也是不连续的。
\chapter{附录}
...
\section{定义}
Definition 1. Through the text, we will use the standard Bachmann-Landau notation, where

\begin{itemize}
    \item $g(\Delta x)=\mathrm{O}\left(\Delta x^{n}\right)$  denotes an upper bound to  $g(\Delta x)$ , that is,  $|g(\Delta x)| \leq C \Delta x^{n}$  for some  $C>0$  as  $\Delta x \rightarrow 0$ .
    \item $g(\Delta x)=\Omega\left(\Delta x^{n}\right)$  denotes a lower bound to  $g(\Delta x)$ , that is,  $|g(\Delta x)| \geq C \Delta x^{n}$  for some  $C>0$  as  $\Delta x \rightarrow 0$.
    \item $g(\Delta x)=\Theta\left(\Delta x^{n}\right)$  denotes the exact order of  $g(\Delta x)$ , that is,  $g(\Delta x)=\mathrm{O}\left(\Delta x^{n}\right)$  and  $g(\Delta x)=\Omega\left(\Delta x^{n}\right)$  as  $\Delta x \rightarrow 0$ .
\end{itemize}


Notice that, by this definition,  $g=\mathrm{O}(1)$  does not mean that  g  is a constant; instead, it means that a constant is an upper bound to  g  as  $\Delta x \rightarrow 0$  or, in other words, that  g  does not asymptotically grow with  $\Delta x$ . For example,  $\Delta x^{2 / 3}=\mathrm{O}(1)$  in this notation.

Definition 2. If  $f\left(x_{c}\right)=f^{\prime}\left(x_{c}\right)=\ldots=f^{\left(n_{\mathrm{cp}}\right)}\left(x_{c}\right)=0$  but  $f^{\left(n_{\mathrm{cp}}+1\right)}\left(x_{c}\right) \neq 0, x_{c}$  is said to be a critical point of order  $n_{\mathrm{cp}}$  of  $f(x)$ . If  $f^{\prime}\left(x_{c}\right) \neq 0, x_{c}$  is defined as a critical point of order 0 of  $f(x)$ .

Definition 3.  $\theta(\psi(\Delta x))$  will denote the order of  $\psi$  (as a function of  $\Delta x$  ), that is, the power of  $\Delta x$  in the leading term of the asymptotic expansion of  $\psi(\Delta x)$ ,
\begin{equation}
...
is $O\left(h^{k+1}\right)$ . Considering (2.4), we need a quadrature whose error is  $O\left(h^{k+l+2}\right)$ , or  $O\left(h^{2 k+2}\right)$ , to work for every  l . For example, Simpson's rule (the error is  $O\left(h^{5}\right)  )$ is enough for  k=1  (second-order scheme), and three-point Gaussian (the error is  $O\left(h^{6}\right)  )$ is needed for  $k=2$  (third-order scheme). Since we can use  $h_{j \pm 1 / 2}$  as  $f\left(u^{h}\left(x_{j \pm 1 / 2}, t\right)\right)$  in the quadratures to save function evaluations, Gauss-Lobatto quadratures, which use end points of the interval as nodes, are preferred.
...
It is accurate for polynomials up to degree 2n – 3, where n is the number of integration points.[8]

Step 1. Here we reconstruct point values of  $u$  at the Gauss or Gauss-Lobatto quadrature points. For the  $\mathbb{P}^{k}$ -based DG (which is  (k+1)  th order accurate), we need a Gauss or Gauss-Lobatto quadrature rule accurate to at least  $\mathrm{O}\left(h^{2 k+2}\right)$ , and the order of accuracy for the WENO reconstruction must be at least  $2 k+1$ . For this purpose, we would need to use the cell averages of the neighboring  2$ k+1$  cells  $I_{i-k}, \ldots, I_{i+k}$  to reconstruct the point values of  u  at the Gauss or Gauss-Lobatto quadrature points. In particular, we have used the following quadrature points:
\begin{itemize}
    \item For the  $\mathbb{P}^{1}$  case, we use the two-point Gauss quadrature points  $x_{i-\sqrt{3} / 6}$  and  $x_{i+\sqrt{3} / 6}$ .
    \item For the  $\mathbb{P}^{2}$  case, we use the four-point Gauss-Lobatto quadrature points  $x_{i-1 / 2}, x_{i-\sqrt{5} / 10}, x_{i+\sqrt{5} / 10}$ , and  $x_{i+1 / 2}$ .
    \item For the  $\mathbb{P}^{3}$  case, we use the four-point Gauss quadrature points  $x_{i-\sqrt{525+70 \sqrt{30}} / 70}, x_{i-\sqrt{525-70 \sqrt{30}} / 70}, x_{i+\sqrt{525-70 \sqrt{30}} / 70}$  and  $x_{i+\sqrt{525+70 \sqrt{30}} / 70}$ .
\end{itemize}
